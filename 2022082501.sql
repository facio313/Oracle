...
- 오라클의 컬럼은 사용자가 데이터를 정의하지 않으면 NULL값이 기본으로 저장
- NULL값과 연산되면 결과는 모두 NULL이 반환됨
- NULL에 관련된 연산자 : IS NULL, IS NOT NULL
NNULL 처리 함수 : : NVL1, NVL2, NULLIF 등이 제공

1) NULL 처리 연산자
- NULL 값은 '='(Equals to)으로 비교 불가능
- 반드시 IS [NOT] NULL로 비교해야 함

사용예) 사원테이블에서 영업실적(COMMISION_PCT)이 NULL이 아닌 사원을 조회하시오.
...
사용예)
2020년 4월 구매회원들에게 특별 마일리지 300포인트를 지급하시오.
UPDATE  MEMBER
